...
If the $X$ variables are observed variables rather than latent factors, the only change refers to the definition of the measurement model. Below, `Lambda = diag(3)` defines three dummy factors with a single indicator loading by 1, so these become observed variables:
...
summary(sem(modelH1, sample.cov = ap$Sigma, 
        ...
        stand = TRUE, fit = TRUE)
```
Note that the `sample.nobs` are arbitrary and that `sample.cov.rescale = FALSE` must be added to prevent `lavaan` from modifying the provided covariance matrix.
