...
The [changelog](https://github.com/moshagen/semPower/blob/master/NEWS) for semPower 2 is available at <https://github.com/moshagen/semPower/blob/master/NEWS>.
...
alpha error, and fewer degrees of freedom. (Note that this only applies 
for effect-size metrics that do not adjust for parsimony.[^1]) When 
performing a power analysis, one of these quantities is computed as 
a function of the other quantities, giving rise to different types of 
power analyses:

[^1]: For instance, when using a constant value for
...
observations to detect an expected effect with the desired power. 
Post-hoc power analyses are performed after data collection to judge whether
the given sample size yields a power that is sufficiently high for a
meaningful test of a certain hypothesis. Post-hoc power analyses are 
not to be confused with "observed power". Estimates of "observed power" 
determine power to detect an effect size observed in a study, which is 
close to meaningless (see [Yuan & Maxwell, 2005](#refs), for 
details). Compromise power analyses are used to determine which decision 
rule to apply in evaluating whether the outcome of a hypothesis test 
favors the null or the alternative hypothesis 
(see [Moshagen & Erdfelder, 2016](#refs), for details).
...
powerCFA <- semPower.powerCFA(
  ...
  # define fitting function (defaults to ML and typically omitted)
  fittingFunction = 'ML',
...
factor model itself). For example (Figure \@ref(fig:figureModCFA)), the following requests the required
...
```{r figureModCFA, echo=FALSE, out.width="75%", fig.align = "left", fig.cap = "A CFA model with two factors that are correlated by .25 in the population. The  path/parameter highlighted in red is targeted by the null hypothesis."}
knitr::include_graphics("fig/cfa.png")
```
...
powerCFA <- semPower.powerCFA(
  ...
  nullEffect = 'corA = corB',
  Phi = list(.2, .4),     # group1, group2
  loadM = c(.7, .5),      # f1, f2 (identical across groups)
  nIndicator = c(5, 5))   # f1, f2 (identical across groups)
```
...
powerCFA <- semPower.powerCFA(
  ...
  nullEffect = 'corA = corB',
  Phi = list(.2, .4, .3),      # group1, group2, group3
...
For example (Figure \@ref(fig:figureModBF)), the following defines a single bifactor with 10 indicators
...
(`c(.70, .75, .80, .85)`). See (Figure \@ref(fig:figureModBF)).

```{r eval=FALSE}
bfLoadings <- rep(.5, 10)    # loadings on general factor
bfWhichFactors <- c(1, 2, 3) # factors being part of bifactor structure
loadings <- list(
...
```{r figureModBF, echo=FALSE, out.width="100%", fig.align = "left", fig.cap = "A bifactor model with one general and three specific factors along with a covariate. The general factor and the covariate are correlated by .30 in the population. The path/parameter highlighted in red is targeted by the null hypothesis. (Residual) variances are not shown."}
knitr::include_graphics("fig/bf.png")
```
...
For example (Figure \@ref(fig:figureModBF)), the following requests the required sample
...
## Regression models {#powerRegression}
...
-   whether the (increase in) the variance explained by one or more predictors 
    differs from zero (`nullEffect = 'R2inc = 0'`).
-   whether the (increase in) the variance explained by one or more predictors 
    is differs across groups (`nullEffect = 'R2incA = R2incB'`).  
...
-   `corXY`: Can be provided instead of `slopes`: Correlation(s) 
    between the predictors and the criterion.
-   `R2base`: Can be provided instead of `slopes` and to be used in 
    conjunction with `R2inc`: Proportion of the variance 
    explained in the criterion by k predictors.
-   `R2inc`:Can be provided instead of `slopes` and to be used in 
    conjunction with `R2base`: Proportion of the incremental variance 
    explained in the criterion by k1 + k2 predictors, where k2 is defined in `nullWhich`.
-   `nullEffect`: Defines the hypothesis of interest; one of
    `'slope = 0'`, `'slopeX = slopeZ'`, `'slopeA = slopeB'`, `'R2inc = 0'`,
    or `'R2incA = R2incB'`.
-   `nullWhich`: Defines which slope(s) is targeted by the hypothesis
    defined in `nullEffect`. When `nullEffect` is `'R2inc = 0'` 
    or `'R2incA = R2incB'`, the maximum number in `nullWhich` defines the total 
    number of predictors, so `nullWhich` must contain the final predictor. For example, 
    when there a total of 5 predictors, `nullWhich` must contain predictor 5.
-   `nullWhichGroups`: Defines which groups are targeted when
...
For example (Figure \@ref(fig:figureModReg)), the following sets up three factors measured by 3, 4, and
...
powerReg <- semPower.powerRegression(
   ...
   type = 'a-priori', alpha = .05, power = .80,
   # define hypothesis
   slopes = c(.2, .3),        # X1 -> Y, X2 -> Y
   corXX = .4, 
   nullEffect = 'slope = 0',
   nullWhich = 1,             # first slope (X1 -> Y)
   # define measurement model
...
```{r figureModReg, echo=FALSE, out.width="100%", fig.align = "left", fig.cap = "A latent regression model with two correlated factors predicting a criterion with (standardized) regression slopes of .2 and .3, respectively. The path/parameter highlighted in red is targeted by the null hypothesis."}
knitr::include_graphics("fig/reg.png")
```
...
powerReg <- semPower.powerRegression(
   ...
   type = 'a-priori', alpha = .05, power = .80,
   # define hypothesis
   slopes = c(.3, .2, .1),      # X1 -> Y, X2 -> Y, X3 -> Y
   corXX = corXX, 
   nullEffect = 'slope = 0',
   nullWhich = 2,               # X2 -> Y
   # define measurement model
   nIndicator = c(3, 3, 3, 3),  # Y, X1, X2, X3
   loadM = c(.5, .5, .5, .5)    # Y, X1, X2, X3
)
```
...
powerReg <- semPower.powerRegression(
   ...
   nullWhich = 1,             # X1 -> Y
   # define measurement model
   nIndicator = c(1, 5, 4),   # Y, X1, X2
   loadM = c(1, .6, .7)       # Y, X1, X2
)
```  
...
Instead of providing individual values for the regression slopes directly 
via the `slopes` argument, it is also possible to provide the (bivariate) 
predictor-criterion correlations via the `corYX` argument. The call above
(with `slopes = c(.2, .3)`) could be replicated via the following call 
(with `corYX = c(.32, .38)`):
...
Again, instead of providing the slopes, one may also  provide the (bivariate) 
predictor-criterion correlations. The call above
(with `slopes = c(.1, .4)`) could be replicated via the following call 
(with `corYX = c(.20, .425)`):
...
powerReg <- semPower.powerRegression(
   ...
   slopes = list(
     # slopes in group 1 
     c(.2, .3, .4),      # X1 -> Y, X2 -> Y, X3 -> Y
     # slopes in group 2 
     c(.2, .05, .4)      # X1 -> Y, X2 -> Y, X3 -> Y
     ),
   corXX = corXX,
   nullEffect = 'slopeA = slopeB',
   nullWhich = 2,
   # define measurement model (same across groups)
   nIndicator = c(4, 5, 3, 6),  # Y, X1, X2, X3
   loadM = c(.5, .6, .7, .6)    # Y, X1, X2, X3
   )
```
...
##### Detect whether the (incremental) variance explained differs from zero {.unnumbered}

To perform a power analysis to detect whether the (incremental) proportion of 
variance explained by a set of predictors differs from zero, use `nullEffect = 'R2inc = 0'`.

When only the proportion of explained variance in $Y$ (i.e. $R^2$) is provided, 
`semPower` computes the slopes by solving 
$$
   b = R_{xx}^{-1} r_{yx} \\
   R^2 = \sum{b \times r_{yx}}
$$
for the slopes ($b$). Depending on the structure of the correlations between the 
predictors ($R_{xx}$), multiple values for the slopes might incur the same $R^2$. 
For example, when there are two uncorrelated predictors (so that $R_{xx} = I$) and 
the target $R^2$ is .25, this could be achieved by any of $b = (.0, .5)$,
$b = (.35356, .35356)$, $b = (.5, 0)$ and many more combinations. 
However, the exact values of the slopes are irrelevant when power
concerning the overall explained variance is of interest. Nevertheless, as
the slopes are estimated, power analyses are often only approximate in these cases,
although still accurate to about 5 decimals.

When defining the effect in terms of $R^2$, one needs to provide both `R2base` 
and `R2inc`. `R2base` defines the $R^2$ for a baseline model with k1 predictors, 
and `R2inc` the incremental $R^2$ yielded by a comparison model involving k1 and k2 other predictors. 
So the total $R^2$ explained by all predictors is $R^2_{tot} = R^2_{base} + R^2_{inc}$.
The evaluated null hypothesis refers to the value defined in `R2inc`. The k2 predictors
added to the k1 predictors of the baseline model are defined in `nullWhich`. 

Consider a total of 3 predictors X1, X2, and X3. The baseline model involves the first two
predictors, $Y = b_1 X_1 + b_2 X_2$ and yields $R^2_{base} = .10$. The comparison
model adds X3 to the regression equation, $Y = b_1 X_1 + b_2 X_2 + b_3 X_3$, and yields an
incremental $R^2_{inc} = .15$, so that $R^2_{tot} = .10 + .15$ . 
This situation is defined using `R2base = .10`, `R2inc = .15`, and `nullWhich = 3`. 
The evaluated null hypothesis refers to whether $R^2_{inc}$ differs from zero.

Similarly, when X1 explains 5% of the variance and one wants to detect that 
adding X2 and X3 incrementally explain 20% of the variance 
beyond X1, this could be achieve by  `R2base = .05`, `R2inc = .20`, 
and `nullWhich = c(2, 3)`. Note that `nullWhich` (implicitly) also defines the 
total number of predictors and thus must always contain the final predictor. So
when there are five predictors in total, `nullWhich` must contain 5.

When the total proportion of explained variance is of interest, define $R^2_{base} = 0$ 
and add all predictors to `nullWhich`. Thus, when there are three predictors that
jointly account for 25% of the variance and a power analysis is performed to detect whether
this proportion differs from zero, use `R2base = 0`, `R2inc = .25`, and `nullWhich = c(1, 2, 3)`.

For example, the following sets up four factors measured by 4, 5, 3, and
6 indicators (`nIndicator = c(4, 5, 3, 6)`). All indicators of the first
factor load by .5, all indicators of the second factor by .6, all of
the third factor by .7, and all of the fourth factor by .6  
(`loadM = c(.5, .6, .7, .6)`; see [definition of a
factor model](#factorDefinition)). Recall that the first factor is
treated as criterion ($Y$), so that there are 3 predictors. 
The correlations between the 3 predictors is defined in `corXX`.
Finally, the required sample (`type = 'a-priori'`) is requested to detect that the 
incremental variance explained (`R2inc = .10`) by the third predictor (`nullWhich = c(3)`) over 
the variance explained by only predictors one and two (`R2base = .05`) differs from zero (`nullEffect = 'R2inc = 0'`) on
alpha = .05 (`alpha = .05`) with a power of 80% (`power = .80`).
...
Similarly, the following asks for the required sample to detect that
all three predictors jointly account for 15% of the variance:
...
powerReg <- semPower.powerRegression(
   ...
   R2base = 0,
   R2inc = .15,
   nullWhich = c(1, 2, 3),      # X1 -> Y, X2 -> Y, X3 -> Y
   # define measurement model
   nIndicator = c(4, 5, 3, 6),  # Y, X1, X2, X3
   loadM = c(.5, .6, .7, .6)    # Y, X1, X2, X3
)
...
##### Detect whether the (incremental) variance explained differs across groups  {.unnumbered}

To perform a power analysis to detect whether the (incremental) proportion of 
variance explained by a set of predictors differs from zero, 
use `nullEffect = 'R2incA = R2incB'`. 

For example, the following sets up four factors in the same way as in the previous
example. Note that the measurement model is identical across groups. The groups are 
equally sized (`N = list(1, 1)`). The $R^2_{base}$
explained by the baseline model involving the first predictor only is 5% in both
groups (`R2base = list(.05, .05)`). The incremental variance explained by predictors 
2 and 3 (`nullWhich = c(2, 3)`) is .025 in the first group and .10 in the second
group (`R2inc = list(.025, .10)`). Finally, the required sample size
is requested to detect that the incremental variance explained differs across
groups (`nullEffect = 'R2incA = R2incB'`).
...
example (Figure \@ref(fig:figureModMed)), the following sets up three factors measured by 3, 4, and 5
...
powerMed <- semPower.powerMediation(
   ...
   nullEffect = 'ind = 0',
   # define measurement model
   nIndicator = c(3, 4, 5),     # X, M, Y
   loadM = c(.5, .6, .7)        # X, M, Y
   )
summary(powerMed)
```

```{r figureModMed, echo=FALSE, out.width="75%", fig.align = "left", fig.cap = "A latent mediation model with an indirect effect of .3*.4 in the population. The path/parameter highlighted in red is targeted by the null hypothesis."}
knitr::include_graphics("fig/med.png")
```
...
(`c(2, 1)`), `M1 -> M2` (`c(3, 2)`), `Y -> M2` (`c(4, 3)`). As 
`estimateDirectEffects` is omitted (and defaults to `TRUE`), all direct effects, 
i.e. `X -> M2`, `X -> Y`, `M1 -> Y`, are also estimated, even though these are
zero in the population. Further, all variables are defined to be observed 
variables rather than latent factors (`Lambda = diag(4)`).

```{r eval=FALSE}
Beta <- matrix(c(
  c(.00, .00, .00, .00),       # X  = .00*X + .00*M1 + .00*M2 + .00*Y 
  c(.20, .00, .00, .00),       # M1 = .20*X + .00*M1 + .00*M2 + .00*Y 
...
powerMed <- semPower.powerMediation(
   ...
   # define hypothesis
   Beta = Beta, 
   indirect = list(c(2, 1), c(3, 2), c(4, 3)),  # .2 * .3 * .4 
   nullEffect = 'ind = 0',
   # define measurement model
   Lambda = diag(4)            # only observed variables
   )
```


Note that `semPower.powerMediation` cannot be used to include 
(residual-)correlations in the model. Suppose there are two `X` variables, 
correlated by .25, and the model considers two possible mediation chains
`X1 -> M -> Y` and `X2 -> M -> Y`. The approach to power analysis 
using `semPower` is to generate the population covariance matrix Sigma 
using [`semPower.powerPath`](#powerPath) and then plug this matrix along with a 
suitable model string into [`semPower.powerLav`](#powerLav).

First, `semPower.powerPath` is used as a handy interface to obtain the population
covariance matrix Sigma. `Beta` defines the regression relations, and `lPsi` 
defines the (residual-)correlations between the variables (when 
`standardized = TRUE`; otherwise it is indeed treated as Psi, 
see [`semPower.powerPath`](#powerPath)). Next, `semPower.powerPath` is called, 
with the sole purpose to obtain Sigma, so all information relating to power are
irrelevant. Then, a `lavaan` model string specifying the H0 model is defined, 
here including the definition of the indirect effect of interest (`ind := a1*b`) 
and restricting it to zero (`ind == 0`). This model string and Sigma are finally
plugged into `semPower.powerLav`, which provides the requested power analysis 
for the hypothesized indirect effect.

```{r eval=FALSE}
# regression relations
Beta <- matrix(c(
  c(.00, .00, .00, .00),       # X1 = .00*X1 + .00*X2 + .00*M + .00*Y
  c(.00, .00, .00, .00),       # X2 = .00*X1 + .00*X2 + .00*M + .00*Y
  c(.20, .30, .00, .00),       # M  = .20*X1 + .30*X2 + .00*M + .00*Y
  c(.10, .05, .25, .00)        # Y  = .10*X1 + .05*X2 + .25*M + .00*Y
), byrow = TRUE, ncol = 4)
...
# use semPower.powerPath to obtain Sigma
powerPath <- semPower.powerPath(
  # define type of power analysis (irrelevant)
  type = 'post-hoc', alpha = .05, N = 1000,
  # define model
  Beta = Beta,
  Psi = lPsi,
  standardized = TRUE,
  # define hypothesis (irrelevant)
  nullEffect = 'beta = 0', 
  nullWhich = c(4, 1),
  # define measurement model
  Lambda = diag(4)
...
# get Sigma and set proper variable names
Sigma <- powerPath$Sigma
colnames(Sigma) <- rownames(Sigma) <- c('X1','X2','M','Y')
...
ind == 0
'
# plug Sigma and model into semPower.powerLav
power <- semPower.powerLav(
  # define type of power analysis
  type = 'a-priori', alpha = .05, power = .80,
  # define hypothesis
  Sigma = Sigma,
...
powerMed <- semPower.powerMediation(
  type = 'a-priori', alpha = .05, power = .80, N = list(1, 1),
  nullEffect = 'indA = indB',
  bYX = list(.25, .25),    # group1, group2
  bMX = list(.2, .4),      # group1, group2
  bYM = list(.3, .5),      # group1, group2
  Lambda = diag(3)         # only observed
 )
```
...
powerMed <- semPower.powerMediation(
  type = 'a-priori', alpha = .05, power = .80, N = list(1, 1),
  nullEffect = 'indA = indB',
  Beta = list(Beta1, Beta2),          # group1, group2
...
details](#semDefinition)). In the example below (Figure \@ref(fig:figureModPath)), there are
...
powerPath <- semPower.powerPath(
  ...
  nullWhich = c(4, 1),
  # define measurement model
  nIndicator = c(3, 4, 5, 6),  # f1, f2, f3, f4
  loadM = c(.7, .5, .6, .8),   # f1, f2, f3, f4
  )
summary(powerPath)
```

```{r figureModPath, echo=FALSE, out.width="100%", fig.align = "left", fig.cap = "A latent path model involving four factors. The (standardized) regression slope between F1 and F4 is .1 in the population. The path/parameter highlighted in red is targeted by the null hypothesis."}
knitr::include_graphics("fig/path.png")
```
...
powerPath <- semPower.powerPath(
  ...
  nullWhich = c(4, 1),
  # define measurement model
  nIndicator = c(1, 4, 1, 6),  # X1, f1, X2, f2 
  loadM = c(1, .5, 1, .8),     # X1, f1, X2, f2   
  )
```
...
powerPath <- semPower.powerPath(
  ...
  type = 'a-priori', alpha = .05, power = .80, N = list(1, 1, 1),
  # define hypothesis
  Beta = list(Beta1, Beta2, Beta3),  # group1, group2, group3
  nullEffect = 'betaA = betaB',
  nullWhich = c(2, 1),
  nullWhichGroups =  c(1, 3),        # restrict across group1 and group3
  # define measurement model (same across groups)
  nIndicator =  c(5, 5),
  loadM =  c(.7, .5)
  )
...
-   `nullEffect`: Defines the level of invariance of interest (see below
    for valid arguments).
-   `useReferenceIndicator`: Defines whether to identify the factor variances and factor means using a referent indicator (`TRUE`) or using a standardization approach setting the means to 0 and the variances to 1 (`FALSE`, the default). See below for details.
-   `singleGroupIdent`: whether to maintain the identification constraints only for a single group (`TRUE`) or whether to maintain all scaling constraints (`FALSE`). See below for details.
-   additional arguments [specifying the type of power
    analysis](#commonArgs).
-   additional arguments [defining the factor model](#factorDefinition),
...
-   `'variances'`: all loadings, (indicator-)intercepts,
    (indicator-)residuals, and factor variances are restricted to
    equality.
-   `'covariances'`: all loadings, (indicator-)intercepts,
    (indicator-)residuals, factor variances, and factor covariances are restricted to
    equality.
-   `'means'`: all loadings, (indicator-)intercepts, and
    (indicator-)residuals, factor variances, factor covariances, and latent means are
...
By default, the factors are identified by setting their means to 0 and their variances to 1 (`useReferenceIndicator = FALSE`), so that all loadings and indicator intercepts are freely estimated. In models involving constraints on loadings or intercepts, a single group identification approach is used by default (`singleGroupIdent = TRUE`), so that the factor variances and means are constrained in only a single group, but are freely estimated in all remaining groups. This mimics the `lavaan` approach to invariance testing. If setting `singleGroupIdent = FALSE`, the identification constraints are maintained in models assuming metric or scalar invariance, so that the factor means and variances are constrained to 0 and 1, respectively, in all groups. Concerning metric invariance, this was the default behavior in `semPower` versions prior 2.1.4.

A referent scaling approach can be be defined using `useReferenceIndicator = TRUE`, so that the loading and the intercept of the first indicator of each factor are constrained in each group, and the factor variances and means are freely estimated. Reference scaling is always maintained in all invariance models, so `singleGroupIdent` must be `FALSE`.
...
For example (Figure \@ref(fig:figureModMI)), the following requests the required sample size (`type = 'a-priori'`) to detect that a metric invariance model (`nullEffect = 'metric'`) differs from a configural invariance model (`comparison = 'configural'`) on alpha = .05 (`alpha = .05`) with a power of 80% (`power = .80`). The model comprises a single factor, which is measured by 5 indicators in both groups. The `loadings` argument specifies that in the first group, all indicators load by .5, whereas in the second group, the indicators load by .7, .4, .6, .4, .7. See the chapter on [specifying a factor model](#factorDefinition) for alternative ways to define the factor loadings. 

Furthermore, in multiple group models, the `N` argument also needs to be supplied as a list, which in case of an a priori power analysis gives the group weights. `N = list(1, 1)` requests equally sized groups. If using `N = list(2, 1)` instead, the first group would be twice as large as the second group. 
...
powerMI <- semPower.powerMI(
   ...
   nullEffect = 'metric',
   # define measurement model
   loadings= list(list(c(.5, .5, .5, .5, .5)), # single factor, group 1
                  list(c(.7, .4, .6, .4, .7))) # single factor, group 2
   )
summary(powerMI)
```

```{r figureModMI, echo=FALSE, out.width="100%", fig.align = "left", fig.cap = "A single factor model measured in two groups. In the H0 model, the loadings are restricted to be equal across groups (highlighted in red)."}
knitr::include_graphics("fig/mi.png")
```
...
powerMI <- semPower.powerMI(
 ...
 # define factor model
 loadings= list(list(c(.5, .5, .5, .5, .5)),  # single factor, group 1
                list(c(.7, .4, .6, .4, .7)))  # single factor, group 1
 )
```
...
The arguments [specifying a factor model](#factorDefinition) must be provided as lists, where each component refers to a specific group. For instance, the following defines a two-factor model, where the first factor is measured by 3 indicators and the second factor is measured by 4 indicators. In the first group, all loadings are equal to .5. In the second group, the loadings on the first factor are also .5, but the loadings on the second factor are .6, .4, .3, and .5 (`loadings = list(list(c(.5, .5, .5), c(.5, .5, .5, .5)), list(c(.5, .5, .5), c(.6, .4, .3, .5)))`). In both groups, the factor correlation is .3 (`Phi = list(.3, 3)`).
...
powerMI <- semPower.powerMI(
   ...
   comparison = 'configural', 
   nullEffect = 'metric',
   # define two factors in two groups
   loadings = list(list(c(.5, .5, .5),        # group 1, factor 1
                        c(.5, .5, .5, .5)),   # group 1, factor 2
                   list(c(.5, .5, .5),        # group 2, factor 1
                        c(.6, .4, .3, .5))),  # group 2, factor 2     
   Phi = list(.3, .3))                        # group1, group2
```

All examples thus far relied on the default identification option (`useReferenceIndicator = FALSE` and `singleGroupIdent = TRUE`). This implies that the factor variances in models involving at least metric invariance are constrained to 1 in a single group, and freely estimated in all remaining groups. Similarly, the factor means in models involving at least scalar invariance are constrained to 0 in a single group, and freely estimated in all remaining groups. In the configural invariance model, factor variances and means are constrained to 1 and 0, respectively, in all groups.

Note that metric invariance equality restrictions are placed on the unstandardized loadings across groups. As a consequence, differences in loadings can be accounted for by the free factor variances, if the loadings are proportional across groups (i.e., the ratio of loadings across groups is identical for all indicators within a factor). In such cases, the metric invariance model shows no discrepancy and a power analysis cannot be performed. For instance, the following assumes a single-factor model, which is measured by 5 indicators in both groups. The loadings are proportional because all indicators load by .5 in the first group, and by .6 in the second group. Running a corresponding power analysis will give an error as both the configural and metric model show an identical and perfect model fit.
...
However, the same code works well when setting `singleGroupIdent = FALSE`. Then, the factor variances are 1 in all groups regardless of the level of invariance, so the factor variances cannot account for proportionally different loadings. 
...
powerMI <- semPower.powerMI(
   ...
   nullEffect = 'scalar',
   # define measurement model
   nIndicator = 5,    # same across groups
   loadM = .5,        # same across groups
...
Similar as in the case of metric invariance, scalar invariance with the default identification option (`useReferenceIndicator = FALSE` and `singleGroupIdent = TRUE`) fixes only the latent factor means of the first group to 0, while the latent factor means of the other groups are set free. As a consequence, differences in indicator intercepts can be accounted for by the free factor means, if the intercepts are proportional across groups (i.e., the ratio of intercepts across groups is identical for all indicators within a factor). In such cases, the scalar invariance model shows no discrepancy and a power analysis cannot be performed. 

For instance, the following assumes a single-factor model, which is measured by 5 indicators in both groups. The intercepts are proportional because all intercepts are 0 in the first group, and .1 in the second group. Running a corresponding power analysis with `useReferenceIndicator = FALSE` and `singleGroupIdent = TRUE` will give an error as both the metric and scalar model show an identical and perfect model fit. However, setting `singleGroupIdent = FALSE` will work fine. 
...
powerMI <- semPower.powerMI(
   ...
   # define measurement model
   nIndicator = 3,                # same across groups
   loadM = .5,                    # same across groups
   Theta = list(Theta1, Theta2))  # group1, group2
```
...
repeated assessment of one or more attributes. The typical - but not in all
...
-   `nullEffect`: Defines the level of invariance of interest (see below
    for valid arguments).
-   `useReferenceIndicator`: Defines whether to identify the factor variances and factor means using a referent indicator (`TRUE`) or using a standardization approach setting the means to 0 and the variances to 1 (`FALSE`, the default).
-   `singleOccasionIdent`: When `useReferenceIndicator = FALSE`, whether to maintain the identification constraints only for a single measurement (`TRUE`) or whether to maintain all scaling constraints (`FALSE`).
-   `autocorResiduals`: Defines whether the residuals of the indicators of latent variables are autocorrelated over waves (`TRUE`, the default) or not (`FALSE`). This affects the df when the comparison model is the saturated model and generally affects power (also for comparisons to the restricted model).
-   `Phi`: Defines the factor covariance matrix, a single number when there are
    exactly two measurements, or `NULL` for uncorrelated factors with unit variance.
-   additional arguments [specifying the type of power
    analysis](#commonArgs).
-   additional arguments [defining the factor model](#factorDefinition).

The general model structure posits at least one factor measured at two ore more occasions. The invariance constraints over occasions are always placed separately for each factor. For example, if two factors are measured twice, equality constraints on loading over time are placed over the measurements of each factor separately. 
...
-   `'variances'`: all loadings, (indicator-)intercepts,
    (indicator-)residual variances, and latent variances are
    restricted to equality.
-   `'covariances'`: all loadings, (indicator-)intercepts,
    (indicator-)residual variances, latent variances, and latent covariances are
    restricted to equality.
-   `'means'`: all loadings, (indicator-)intercepts,
    (indicator-)residual variances, latent variances, latent covariances, and latent means
    are restricted to equality.
...
-   `c('loadings', 'intercepts', 'means')`: all loadings,
    (indicator-)intercepts, and latent factor means are restricted to
    equality.
-   `c('loadings', 'residuals', 'lv.variances')`: all loadings,
    (indicator-)residual variances, and latent factor variances are
    restricted to equality.
...
A referent scaling approach can be be defined using `useReferenceIndicator = TRUE`, so that the loading and the intercept of the first indicator of each factor are constrained at all occasions, and the factor variances and means are freely estimated. Reference scaling is always maintained in all invariance models, so `singleOccasionIdent` has no effect (i.e. is always `FALSE`).

Beyond the arguments stated above, `semPower.powerLI` requires the [definition of the factor model](#factorDefinition). In particular, either `Lambda`, or `loadings`, or `nIndicator` and `loadM` always need to be defined, and `Theta`, `tau` and `Alpha` need to be defined for particular levels of invariance. If longitudinal invariance for a single factor measured at T occasions is of interest, `loadings` can be a single list with T elements, and `nIndicator`, `loadM`, `tau` and `Alpha` can be single vectors. If longitudinal invariance for multiple factors measured at T occasions is of interest, `loadings`, `nIndicator`, `loadM`, `tau` and `Alpha` must be wrapped in a list with T elements, where each element refers to a single measurement occasion. `Phi` and `Theta` always must be a single matrix. The order of `Phi` must be F1_T1, F2_T1, ..., F1_T2, F2_T2, ..., F1_TZ, F2_TZ, ..., FX_TZ. The order of `Theta` must match the order of `Phi` and thus first contains the respective indicators of F1 at T1, F2 at T1, ..., F1 at T2, F2 at T2, and so on.
...
For example (Figure \@ref(fig:figureModLI)), the following requests the required sample size
...
attribute is measured by 4 indicators (at both the first and the second
measurement). At the first measurement, all indicators load by .7, 
whereas at the second measurement, the indicators
load by .4, .5, .6, and .6. In addition, the model comprises
autocorrelated indicator residuals across time, because the
`autocorResiduals` (which defaults to `TRUE`) argument is omitted, and
relies on single occasion identification 
(by omitting `useReferenceIndicator = FALSE` and `singleOccasionIdent = TRUE`).
...
```{r figureModLI, echo=FALSE, out.width="90%", fig.align = "left", fig.cap = "A single factor measured at two occasions. The model includes  autocorrelated indicator residuals  (which are all zero in the population). In the H0 model, the loadings are restricted to be equal across groups (highlighted in red)."}
knitr::include_graphics("fig/li.png")
```
...
powerLI <- semPower.powerLI(
  ...
  type = 'a-priori', alpha = .05, power = .80,
  # define hypothesis
  comparison = 'none',    # configural invariance
  nullEffect = 'loadings',
  # define model
  loadings = list(
    c(.7, .7, .7, .7),   # factor 1 at T1
...
Note that by default the factors are scaled using single occasion identification 
(`useReferenceIndicator = FALSE` and `singleOccasionIdent = TRUE`). This means that 
the variance of the factor(s) measured at the first occasion are fixed to 1, 
whereas the remaining factor variances are freely estimated. All loadings are also
freely estimated. 

Repeating the call above but adding `singleOccasionIdent = FALSE` would
again freely estimate all loadings, but fix all factor variances to 1. These
different approaches to identification can have a profound impact on power, also
because this changes the df.

A reference identification results by adding `useReferenceIndicator = TRUE` (and 
`singleOccasionIdent = FALSE`). Then, all factor variances are freely estimated, whereas 
the loading of the first indicator of each factor is fixed. Thus, the loadings of 
the first indicator should be equal across measurements.

In the examples above, only a single factor measured repeatedly was considered.
If more factors are measured, the `loadings` argument must be put in a list 
(as must other options to [specify the factor model](#factorDefinition), except for `Lambda`).
For instance, the following defines a
two-factor model measured by three indicators each at two time points.
At the first measurement occasion, the loadings on the first factor are .8, .8, and .7 
and those on the second factor are .5, .7, and .5. At the second measurement occasion 
the loadings on the first factor are .7, .6, .7, and those on the second factor are
.6, .8, and .4. In addition, because the factor is
measured at three time points, `Phi` now becomes a factor covariance
matrix, in this case specifying all variances to be 1, 
the correlations between the first and second measurement of the first factor to be .5,
the correlation between the first and second measurement the second factor to be .6, 
and all other correlations to be zero.
...
powerLI <- semPower.powerLI(
  ...
  loadings = list(
    list(
      c(.8, .8, .7),   # factor 1 at T1
      c(.5, .7, .5)    # factor 2 at T1
    ),
    list(
      c(.7, .6, .7),   # factor 1 at T2
      c(.6, .8, .4)    # factor 2 at T2
    )
  ),
  Phi = Phi
)
...
time points, which is measured by 4 indicators that load by .7, .6, .7, and .6 
at both measurement occasions (`loadings`; see [definition of
the factor model](#factorDefinition)). Importantly, the indicator
intercepts (`tau`) partly differ across time: the intercepts of the
second and fourth indicators is 0 at the first second measurement, but .2 at the
...
powerLI <- semPower.powerLI(
  ...
  # define model
  loadings = list(
    c(.7, .6, .7, .6),    # factor 1 at T1
    c(.7, .6, .7, .6)     # factor 1 at T2
  ),
  tau = c(0, 0, 0, 0,     # intercepts at T1
          0, .2, 0, .2),  # intercepts at T2
  Phi = .3
)
```
...
Again note that different identification options may lead to different results.
By default, single occasion identification 
(`useReferenceIndicator = FALSE` and `singleOccasionIdent = TRUE`) freely estimates 
all intercepts and all factor means except for those measured at the first occasion.
`singleOccasionIdent = FALSE` also freely estimate all intercepts, but fixes all factor means to 0. 
A reference identification (`useReferenceIndicator = TRUE`) freely estimates
all factor means and fixes the intercept of the first factor indicator to 0.

If multiple factors are measured repeatedly, `tau` becomes a list with each
element providing the intercepts at a particular measurement, as in the following example:
...
powerLI <- semPower.powerLI(
  ...
  loadings = list(
    ...
    # loadings at T2
    list(
      c(.8, .8, .7),   # factor 1 at T2
      c(.5, .7, .5)    # factor 2 at T2
    )
  ),
  tau = list(
    # intercepts at T1
...
occasion, but .25 at the second measurement occasion:
...
Again, if multiple factors are measured repeatedly, both `tau` and `Alpha` become a list.
...
##### Detect whether factor variances differ across measurements {.unnumbered}

Detecting that factor variances differ across time again proceeds
largely identical to the procedure described above for tests of other
levels of invariance. However, the model definition must include a
definition of the factor variance-.covariance matrix (`Phi`).

For instance, the following requests the required sample
(`type = 'a-priori'`) to detect that a metric invariance model with invariant variances
(`nullEffect = c('loadings', 'lv.variances')`) differs from a metric invariance model without
constraints on the variances (`comparison = c('loadings')`) on alpha = .05 (`alpha = .05`) with a power of
80% (`power = .80`). The model comprises an attribute assessed at two
time points, which is measured by 4 indicators that load by .7, .6, .7, and .6 
at both measurement occasions. The factor variance-covariance matrix (`Phi`) defines
the variance of the factor at the first measurement to be 1, but at the
second measurement to be .7.
...
powerLI <- semPower.powerLI(
  ...
  # define hypothesis
  comparison = c('loadings'), 
  nullEffect = c('loadings', 'lv.variances'),
  # define model
  loadings = list(
    c(.7, .6, .7, .6),   # factor 1 at T1
    c(.7, .6, .7, .6)    # factor 1 at T2
...
largely identical to the procedure described above for tests of other
levels of invariance. The tests of covariances only refer to covariances between
factors measured at the same occasion, i.e. covariances across measurements (such as stabilities) not part of this hypothesis.

For instance, the following requests the required sample
(`type = 'a-priori'`) to detect that the factor covariances differ
across time (`nullEffect = c('loadings', 'lv.variances', 'lv.covariances')`) in
comparison to a metric invariance model with equal variances (`comparison = c('loadings', 'lv.variances')`)
on alpha = .05 (`alpha = .05`) with a power of 80% (`power = .80`). The
correlation (because the variances are 1) between the first and second factor 
at the first measurement is .2, whereas the correlation between the first and 
the second factor at the second measurement is 0. The null hypothesis states that these two
...
powerLI <- semPower.powerLI(
  type = 'a-priori', alpha = .05, power = .80,
  # define hypothesis
  comparison = c('loadings', 'lv.variances'), 
  nullEffect = c('loadings', 'lv.variances', 'lv.covariances'),
  # define model
  loadings = list(
    list(
      c(.8, .8, .7),   # factor 1 at T1
...
(i.e, loadings, intercepts, residuals, variances).
...
For example (Figure \@ref(fig:figureModautoreg)), the following requests the required sample size
...
powerAutoreg <- semPower.powerAutoreg(
  ...
  # define hypothesis 
  nWaves = 3,
  autoregEffects = c(.5, .7),  # f1->f2, f2->f3
  nullEffect = 'autoreg = 0',
  nullWhich = 1,
  # define measurement model
  nIndicator = c(3, 3, 3),     # f1, f2, f3
  loadM = .5
)
summary(powerAutoreg)
```

```{r figureModautoreg, echo=FALSE, out.width="100%", fig.align = "left", fig.cap = "An autoregressive model involving a single factor measured at three occasions with (standardized) autoregressive effects in the population of .5 from the first to the second and of .7 from the second to the third measurement. The model includes autocorrelated indicator residuals (which are all zero in the population). The path/parameter highlighted in red is targeted by the null hypothesis."}
knitr::include_graphics("fig/autoreg.png")
```
...
powerAutoreg <- semPower.powerAutoreg(
  ...
  # define hypothesis 
  nWaves = 3,
  autoregEffects = c(.5, .5),  # f1->f2, f2->f3
  waveEqual = c('autoreg'),
  nullEffect = 'autoreg = 0',
  # define measurement model
  nIndicator = c(3, 3, 3), loadM = .5
...
powerAutoreg <- semPower.powerAutoreg(
  ...
  # define hypothesis 
  nWaves = 4,
  lag1Effects = c(.5, .5, .5),     # f1->f2, f2->f3, f3->f4
  lag2Effects = c(.2, .2),         # f1->f3, f2->f4
  lag3Effects = .1,                # f1->f4
...
powerAutoreg <- semPower.powerAutoreg(
  ...
  # define hypothesis 
  nWaves = 3,
  autoregEffects = c(.5, .6),  # f1->f2, f2->f3
  nullEffect = 'lag1',
  # define measurement model
  nIndicator = c(3, 3, 3), loadM = .5,
  standardized = FALSE
...
powerAutoreg <- semPower.powerAutoreg(
  ...
  nWaves = 3,
  autoregEffects = list(
    c(.5, .5),  # group 1: f1->f2, f2->f3
    c(.4, .4)   # group 2: f1->f2, f2->f3    
  ),
  waveEqual = c('autoreg'),
...
-   `waveEqual`: Parameters that are assumed to be equal across waves in
    ...
    for the variance of the noise factors $N_2 \ldots N_{nWaves}$ , and
    `mean` for the conditional means of $X_2 \ldots X_{nWaves}$ (starting
...
For example (Figure \@ref(fig:figureModARMA)), the following requests the required sample size
(`type = 'a-priori'`) to detect that the autoregressive lag-1 effect
...
identify the model, the `waveEqual` argument is set so that the
 moving average effects and the variances are restricted to be equal across measurements (excluding the
first wave) in both the H0 and the H1 model (`waveEqual = c('mvAvg', 'var')`). In
...
powerARMA <- semPower.powerARMA(
  ...
  # define hypothesis 
  nWaves = 5,
  autoregLag1 = c(.5, .4, .3, .2),  # f1->f2, f2->f3, f3->f4, f4->f5 
  mvAvgLag1 = c(.4, .4, .4, .4),    # n1->f2, n2->f3, n3->f4, n4->f5 
  variances = c(1, 1, 1, 1, 1),     # n1, n2, n3, n4, n5
  waveEqual = c('var', 'mvAvg'),
  nullEffect = 'autoreg',
  # define measurement model
  nIndicator = c(3, 3, 3, 3, 3),   #  f1, f2, f3, f4, f5
  loadM = .5
)
summary(powerARMA)
```

```{r figureModARMA, echo=FALSE, out.width="100%", fig.align = "left", fig.cap = "A latent ARMA model involving 5 waves. The (lag 1) autoregressive effects are  .5, .4, .3, and .2 in the population. The model includes autocorrelated indicator residuals (which are all zero in the population). The H0 model restricts the autoregressive effects to equality (highlighted in red). (Residual) variances and intercepts/means are not shown."}
knitr::include_graphics("fig/arma.png")
```
...
powerARMA <- semPower.powerARMA(
  ...
  # define hypothesis 
  nWaves = 5,
  autoregLag1 = c(.4, .4, .4, .4),  # f1->f2, f2->f3, f3->f4, f4->f5 
  autoregLag2 = c(.15, .1, .05),    # f1->f3, f2->f4, f3->f5 
  autoregLag3 = c(.05, .10),        # f1->f5, f2->f5 
  mvAvgLag1 = c(.3, .3, .3, .3),    # n1->f2, n2->f3, n3->f4, n4->f5 
  variances = c(1, 1, 1, 1, 1),     # n1, n2, n3, n4, n5
...
to the example described above, but restricts the autoregressive effects 
to be equal and requests the required
...
powerARMA <- semPower.powerARMA(
  ...
  nWaves = 5,
  autoregLag1 = c(.5, .4, .3, .2),  # f1->f2, f2->f3, f3->f4, f4->f5 
  mvAvgLag1 = c(.3, .4, .5, .4),    # n1->f2, n2->f3, n3->f4, n4->f5 
  variances = c(1, 1, 1, 1, 1),     # n1, n2, n3, n4, n5
...
powerARMA <- semPower.powerARMA(
  ...
  nWaves = 5,
  autoregLag1 = c(.4, .4, .4, .4),  # f1->f2, f2->f3, f3->f4, f4->f5 
  autoregLag2 = c(.1, .1, .1),      # f1->f3, f2->f4, f3->f5 
  autoregLag3 = c(.05, .05),        # f1->f5, f2->f5 
  mvAvgLag1 = c(.3, .3, .3, .3),    # n1->f2, n2->f3, n3->f4, n4->f5 
  mvAvgLag2 = c(.20, .10, .05),     # n1->f3, n2->f4, n3->f5 
  mvAvgLag3 = c(.05, .10),          # n1->f5, n2->f5 
  variances = c(1, 1, 1, 1, 1),     # n1, n2, n3, n4, n5
  waveEqual = c('var', 'autoreg', 'autoregLag2', 'autoregLag3', 'mvAvg'),
  nullEffect = 'mvAvgLag2',
  # define measurement model
...
powerARMA <- semPower.powerARMA(
  ...
  # define hypothesis 
  nWaves = 5,
  autoregLag1 = c(.5, .5, .5, .5),  # f1->f2, f2->f3, f3->f4, f4->f5  
  mvAvgLag1 = c(.3, .3, .3, .3),    # n1->f2, n2->f3, n3->f4, n4->f5 
...
powerARMA <- semPower.powerARMA(
  ...
  mvAvgLag1 = c(.3, .3, .3, .3),    # n1->f2, n2->f3, n3->f4, n4->f5  
  variances = c(1, 1, 1, 1, 1),     # n1, n2, n3, n4, n5
  means = c(0, .3, .2, .5, .4),     # f1, f2, f3, f4, f5
...
powerARMA <- semPower.powerARMA(
  ...
  autoregLag1 = c(.4, .4, .4, .4),  # f1->f2, f2->f3, f3->f4, f4->f5 
  mvAvgLag1 = c(.3, .3, .3, .3),    # n1->f2, n2->f3, n3->f4, n4->f5
  mvAvgLag2 = c(.2, .2, .2),        # n1->f3, n2->f4, n3->f5 
  variances = c(1, 1, 1, 1, 1),     # n1, n2, n3, n4, n5
...
estimated in each group by default). The measurement model must be identical
across groups, so always provide the measurement model 
for a single group only (do not use a list structure).
...
powerARMA <- semPower.powerARMA(
  ...
  nWaves = 5,
  autoregLag1 = list(
    c(.5, .5, .5, .5),  # group 1: f1->f2, f2->f3, f3->f4, f4->f5    
    c(.3, .3, .3, .3)   # group 2: f1->f2, f2->f3, f3->f4, f4->f5     
  ),
  mvAvgLag1 = c(.3, .3, .3, .3),    # same across groups: n1->f2, n2->f3, n3->f4, n4->f5 
  variances = c(1, 1, 1, 1, 1),     # same across groups: n1, n2, n3, n4, n5
  waveEqual = c('var', 'autoreg', 'mvAvg'),
  groupEqual = c('var'),
  nullEffect = 'autoregA = autoRegB',
  # define measurement model
...
powerARMA <- semPower.powerARMA(
  ...
  # define hypothesis 
  nWaves = 5,
  autoregLag1 = c(.5, .5, .5, .5), # same across groups: f1->f2, f2->f3, f3->f4, f4->f5 
  mvAvgLag1 = list(
    c(.5, .5, .4, .3),  # group 1: n1->f2, n2->f3, n3->f4, n4->f5     
    c(.5, .2, .4, .3)   # group 2: n1->f2, n2->f3, n3->f4, n4->f5      
  ),
  variances = c(1, 1, 1, 1, 1),     # n1, n2, n3, n4, n5
  waveEqual = c('var', 'autoreg'),
  groupEqual = c('var'),
...
powerARMA <- semPower.powerARMA(
  ...
  mvAvgLag1 = c(.3, .3, .3, .3),    # same across groups: n1->f2, n2->f3, n3->f4, n4->f5 
  variances = list(
    c(1, 1, 1, 1, 1),         # group 1: n1, n2, n3, n4, n5
    c(.6, .6, .6, .6, .6)     # group 2: n1, n2, n3, n4, n5
  ),
...
powerARMA <- semPower.powerARMA(
  ...
  variances = c(1, 1, 1, 1, 1),     # same across groups: n1, n2, n3, n4, n5
  means = list(
    c(0, 0, 0, 0, 0),         # group 1: n1, n2, n3, n4, n5
    c(0, .5, .5, .5, .5)      # group 2: n1, n2, n3, n4, n5
  ),
...
-   `autoregEffects`: Vector of the autoregressive effects of $X$ and
    ...
    effects for $X$ and $Y$ from wave to wave, e.g. `list(c(.7, .6), c(.5, .5))` for a autoregressive effect of .7 for `X1 -> X2` and .6 for `X2 -> X3` and autoregressive effects of .5 for `Y1 -> Y2` and `Y2 -> Y3`.
-   `crossedEffects`: Vector of cross-lagged effects of $X$ on $Y$
    (`X -> Y`) and of $Y$ on $X$ (`Y -> X`) (both constant across
    waves), or a list of vectors of cross-lagged effects for each wave, e.g. 
    `list(c(.2, .3), c(.1, .1))` for `⁠X1 - > Y2`⁠ = .2, `X2 -> Y3` = .3, `Y1 -> Y2` = .1, and `Y2 -> Y3` = .1.
...
`crossedEffects = list(c(.1, .2), c(.3, .4))`, the system becomes (Figure \@ref(fig:figureModCLPMi)):

$$
\hat{X_2} = .8 \cdot X_1 + .3 \cdot Y_1 \\
\hat{Y_2} = .1 \cdot X_1 + .6 \cdot Y_1 \\
\hat{X_3} = .7 \cdot X_2 + .4 \cdot Y_2 \\
\hat{Y_3} = .2 \cdot X_2 + .5 \cdot Y_2
$$ 

```{r figureModCLPMi, echo=FALSE, out.width="75%", fig.align = "left", fig.cap = "A CLPM with two attributes measured at three waves."}
knitr::include_graphics("fig/clpmi.png")
```
...
For example (Figure \@ref(fig:figureModCLPM)), the following sets up a CLPM with two waves (`nWaves = 2`)
...
powerCLPM <- semPower.powerCLPM(
  ...
  nWaves = 2,
  autoregEffects = c(.60, .70),   # X1 -> X2, Y1 -> Y2
  crossedEffects = c(.10, .15),   # X1 -> Y2, Y1 -> X2
  rXY = c(.3, .1),                # X1 <-> Y1, X2 <-> Y2 
  # define measurement model
  nIndicator = c(5, 3, 5, 3),     # X1, Y1, X2, Y2
  loadM = c(.5, .6, .5, .6)       # X1, Y1, X2, Y2
  )
summary(powerCLPM)
```

```{r figureModCLPM, echo=FALSE, out.width="100%", fig.align = "left", fig.cap = "A CLPM with two attributes measured twice each (i.e, two waves). In the population, the (standardized) cross-lagged effect of X on Y is .1, and the one of Y on X is .15. The model includes autocorrelated indicator residuals (which are all zero in the population). The path/parameter highlighted in red is targeted by the null hypothesis."}
knitr::include_graphics("fig/clpm.png")
```
...
powerCLPM <- semPower.powerCLPM(
  ...
  nullEffect = 'crossedX = 0',
  nWaves = 3,
  autoregEffects = c(.60, .70),  # X_s -> X_t, Y_s -> Y_t
  crossedEffects = c(.10, .15),  # X_s -> Y_t, Y_s -> X_t
  rXY = c(.3, .1, .1),           # X1 <-> Y1, X2 <-> Y2, X3 <-> Y3 
...
powerCLPM <- semPower.powerCLPM(
    ...
    nWaves = 2,
    autoregEffects = c(.50, .80),   # X1 -> X2, Y1 -> Y2
    crossedEffects = c(.10, .05),   # X1 -> Y2, Y1 -> X2
    rXY = c(.2, .3),                # X1 <-> Y1, X2 <-> Y2
    # define measurement model
...
powerCLPM <- semPower.powerCLPM(
  ...
  # define measurement model
  nIndicator = c(10, 8, 10, 8, 10, 8),   # X1, Y1, X2, Y2, X3, Y3 
  loadM = c(.7, .6, .7, .6, .7, .6)      # X1, Y1, X2, Y2, X3, Y3  
  )
```
...
powerCLPM <- semPower.powerCLPM(
  ...
  crossedEffects = c(.05, .15),         # X_s -> Y_t, Y_s -> X_t
  rXY = c(.3, .2, .1),                  # X1 <-> Y1, X2 <-> Y2, X3 <-> Y3  
...
powerCLPM <- semPower.powerCLPM(
  ...
  waveEqual = c('autoregX', 'autoregY', 'crossedX', 'crossedY'),
  rXY = NULL,           # uncorrelated
  # define measurement model (same across groups)
  nIndicator = c(5, 3, 5, 3, 5, 3),   # X1, Y1, X2, Y2, X3, Y3
  loadM = c(.5, .4, .5, .4, .5, .4)   # X1, Y1, X2, Y2, X3, Y3
  )
```
...
powerCLPM <- semPower.powerCLPM(
  ...
  rXY = c(.3, .3, .3),  # same across groups
  # define measurement model (same across groups)
...
For example (Figure \@ref(fig:figureModRICLPM)), the following sets up a RI-CLPM with three waves
(`nWaves = 3`) and requests the required sample size
...
powerRICLPM <- semPower.powerRICLPM(
  ...
  nullEffect = 'corBXBY = 0',
  nWaves = 3,
  autoregEffects = c(.5, .4),   # X_s -> X_t, Y_s -> Y_t 
  crossedEffects = c(.2, .1),   # X_s -> Y_t, Y_s -> X_t
  rXY = NULL,                   # diagonal
  rBXBY = .30,                  
  # define measurement model
  nIndicator = rep(3, 6),
...
```{r figureModRICLPM, echo=FALSE, out.width="100%", fig.align = "left", fig.cap = "A latent RI-CLPM with two attributes measured at three waves. In the population, the correlation between the intercept factors of X and Y is .3. The model includes autocorrelated indicator residuals (which are all zero in the population). The path/parameter highlighted in red is targeted by the null hypothesis. (Residual) variances and intercepts/means are not shown."}
knitr::include_graphics("fig/riclpm.png")
```
...
powerRICLPM <- semPower.powerRICLPM(
  ...
  nullWhich = 2,
  nWaves = 3,
  autoregEffects = c(.5, .4),   #  X_s -> X_t, Y_s -> Y_t; constant across waves and groups
  crossedEffects = list(
  # group 1
  list(
    c(.25, .10),    # X1 -> Y2, X2 -> Y3 
...
For example (Figure \@ref(fig:figureModLGCM)), the following requests the required sample size
...
```{r figureModLGCM, echo=FALSE, out.width="100%", fig.align = "left", fig.cap = "A latent LGCM with a factor measured on three occasions (i.e., 3 waves). In the population, the means of the intercept and slope factors are .5 and .2, respectively. The model includes autocorrelated indicator residuals (which are all zero in the population). The path/parameter highlighted in red is targeted by the null hypothesis. (Residual) variances are not shown."}
knitr::include_graphics("fig/lgcm.png")
```
...
powerLGCM <- semPower.powerLGCM(
  ...
  variances = c(1, 1),  # i, s (same across groups)
  covariances = .3,
...
powerLGCM <- semPower.powerLGCM(
  ...
  # define hypothesis 
  nWaves = 3,
  means = c(.5, .2),    # i, s (same across groups)
  variances = list(
    c(.5, 1),           # group 1: i, s
    c(.5, .5)           # group 2: i, s
  ),         
...
powerLGCM <- semPower.powerLGCM(
  ...
  ticExogSlopes = list(
    c(.5, .1),           # group 1: cov -> i, cov -> s
    c(.1, .2)            # group 2: cov -> i, cov -> s
  ),
  nullEffect = 'betaITA = betaITB',
  # define measurement model (same for all groups)
  loadings = list(
...
powerLGCM <- semPower.powerLGCM(
  ...
  groupEqual = c('iVar', 'sVar', 'isCov'),
  ticEndogSlopes = list(
    c(.3, .2),           # group 1: i -> cov, s -> cov
    c(.1, .5)            # group 2: i -> cov, s -> cov
  ),
  nullEffect = 'betaTSA = betaTSB',
  # define measurement model (same for all groups)
  loadings = list(
...
semPower.powerPlot.byEffect(
  effect.measure = 'RMSEA', alpha = .05, N = 500, 
...
-   `nCores`: The number of CPU cores to use for parallel processing.
    ...
    is installed.
-   `futureStrategy`: How to set up the backend for parallel processing (when 
    `nCores` is > 1). Defaults to `'multisession'`. This is passed to 
    the `plan` function of the `doFuture` package. See the `doFuture` package  
    for valid strategies.
...
plot(dchisq(seq(max(powerCFA$simRes$fitH1[ , 'chisq'])), df = powerCFA$simRes$fitH1[1 , 'df']), 
     type = 'l', lty = 2, xlab = 'Chi-Square', ylab = 'Density')
...
# get covariance matrix as implied by H0 model; note the sample.nobs are arbitrary
...
```{r eval=FALSE}
ph <- semPower.postHoc(alpha = .05, N = 250, df = df,
                       SigmaHat = covH0, Sigma = covPop)
summary(ph)
```
...
```{r eval=FALSE}
ph <- semPower.powerLav(
  type = 'post-hoc', alpha = .05, N = 250,
...
X_4 = 0.4 \cdot X_1 +  0.5 \cdot X_2 + 0.0 \cdot X_3 + 0.0 \cdot X_4
$$
...
covariance between $X_1$ and $X_2$ is .6 (which is also a correlation,
because both variances are 1) and a residual covariance between $X_3$
and $X_4$ of .1 (which is *not* a correlation, because the variances of
$X_3$ and $X_4$ also depend on Beta).

The following Figure (\@ref(fig:figureModCFA)) shows the resulting structure:

```{r figureModGP, echo=FALSE, out.width="50%", fig.align = "left", fig.cap = "Structural relations among four variables."}
knitr::include_graphics("fig/pathx.png")
```
...
-   Yuan, K.-H., & Maxwell, S. (2005). On the post hoc power in 
    testing mean differences. *Journal of Educational and Behavioral 
	Statistics, 30*, 141–167.
