...
If a certain indicator should only load on the specific factor, but not on the bifactor, define the respective loading on the bifactor to equal zero. In the following example, the first indicator of the specific factor (and thus the second indicator of the bifactor) only loads in the specific factor:
...
The same approach is used for more complex structures involving more than one bifactor and more than one covariate. If more than one bifactor is to be defined, `bfLoadings` and `bfWhichFactors` become lists, where each component refers to a particular bifactor. For instance, the following defines two bifactors. The first involves 10 indicators all loadings by .6 (`rep(.6, 10)`), the second involves 11 indicators all loadings by .6 (`rep(.6, 11)`). Both bifactors comprise 3 specific factors (`bfWhichFactors`), the first factors 1-3 (`c(1, 2, 3)`) and the second factors 4-6 (`c(4, 5, 6)`). The `loadings` argument shows that the specific factors 1-3 (that are part of the first bifactor) all involve three indicators, whereas the specific factors 4-6 (that are part of the second bifactor) involve 4, 3, and 3 indicators, respectively. Further, `loadings` also defines a factor that is not part of any of the bifactors and thus act as a covariate. The covariate is a factor with 4 indicators, all loading by .6 (`c(.6, .6, .6, .6)`). As there are now two bifactors and one additional covariate, `Phi`, defining their intercorrelations, must be a $3 \cdot 3$ matrix, where the columns 1-2 refer to the bifactors, and the third column to the covariate. The correlations in `Phi` imply that the correlations between the bifactors is .3. The covariate correlates by .5 and .1 to the first and second bifactor, respectively.
...
To perform a power analysis to detect whether a correlation between a bifactor and another factor or observed covariate differs from zero, use `nullEffect = 'cor = 0'` (which is also the default hypothesis and thus could be omitted). 

For instance, the following requests the required sample (`type = 'a-priori'`) to detect that a correlation between the bifactor and the covariate of at least .30 (`Phi`) differs from zero (`nullEffect = 'cor = 0'`) on alpha = .05 (`alpha = .05`) with a power of 80% (`power = .80`). `nullWhich = c(1, 2)` defines which correlation between the factors should be restricted to zero. In this example, there is only a single correlation in `Phi`, so `nullWhich` must be `c(1, 2)`. The arguments related to the definition of the bifactor structure were described in detail in the previous section, implying a bifactor spanning 10 indicators, three specific factors with 3 indicators each, and a covariate not part of the bifactor structure with 4 indicators. 
...
summary(powerBF$power)
```
The results of the power analysis are printed by calling the `summary` method on `powerBF$power`, which in this example provides the same information as a model-free [a-priori power analysis](#ap) counterpart.
...
```
Now, `summary(powerBF$power)` provides the same information as a model-free [post-hoc power analysis](#ph) counterpart. A  [compromise power analysis](#cp) (`type = 'compromise'`) is performed analogously.
...
For instance, the following requests the required sample (`type = 'a-priori'`) to detect that a correlation between the bifactor and the first covariate (of .30) differs from the correlation of between the bifactor and the second covariate (of .50; `nullEffect = 'corX = corZ'`) on alpha = .05 (`alpha = .05`) with a power of 80% (`power = .80`). The arguments related to the definition of the bifactor structure were described in detail in one of the previous sections, implying a bifactor spanning 10 indicators, three specific factors with 3 indicators each, and two covariates that are not part of the bifactor structure with 4 and 5 indicators, respectively. 
...
Note that `nullWhich` is now a list comprising two vectors, jointly defining which correlations defined in `Phi` to set to equality. `nullWhich = list(c(1, 2), c(1, 3))` says that the correlation between the first and the second factor (`c(1, 2)`) and the correlation between the first and the third (`c(1, 3)`) factor are restricted to be equal. Since the first factor in `Phi` refers to the bifactor, this means that the correlation of the bifactor to the two covariates is targeted by the null hypothesis.
...
If there are more than two groups, the targeted correlation is held equal across all groups by default. If the correlation should only be constrained to equality in specific groups, `nullWhichGroups` is used to identify the groups to which the equality restrictions apply. For instance, the following expands the previous example by defining another `Phi` for a third group and then asks for the required sample to detect that the correlation in group 1 differs from the one in group 3 (`nullWhichGroups = c(1, 3)`). 
...
* whether the crossed effect of $X$ on $Y$ (`crossedXA = crossedXB`) or the crossed effect of $Y$ on $X$ (`crossedYA = crossedYB`) differs across groups. 
...
* `nullWhichGroups`: for hypothesis involving cross-groups comparisons, vector indicating the groups for which equality constrains should be applied.
...
`semPower.powerCLPM` assumes that two variables $X$ and $Y$ are repeatedly assessed (a CLPM involving more than two variables can be more laboriously specified as a [generic path model](#powerPath)). The structure of a CLPM is defined by specifying `nWaves`, `autoregEffects`, `crossedEffects`,  `rXY`, and `standardized`.

`nWaves` defines the number of measurements of $X$ and $Y$ and thus must be two or larger. Setting `nWaves = 2` implies the following regression equations: 
$$
\hat{X_2} = \beta_{X_2,X_1} \cdot X_1 + \beta_{X_2, Y_1} \cdot Y_1 \\
\hat{Y_2} = \beta_{Y_2,X_1} \cdot X_1 + \beta_{Y_2, Y_1} \cdot Y_1
$$
So that $\beta_{X_2,X_1}$ is the autogressive effect (the stability) of $X$ (`X1 -> X2`),  $\beta_{Y_2, Y_1}$  the autogressive effect of $Y$ (`Y1 -> Y2`),  $\beta_{X_2, Y_1}$ the cross-lagged effect of $Y$ on $X$ (`Y1 -> X2`), and $\beta_{Y_2, X_1}$ the cross-lagged effect of $X$ on $Y$ (`X1 -> Y2`). The correlation between $X_1$ and $Y_1$ (`X1 <-> Y1`) as well as the residual correlation between $X_2$ and $Y_2$ (`X2 <-> Y2`; the synchronous effects) are defined in `rXY`, which defaults to `NULL` implying zero correlations. If setting `rXY = c(.3, .5)` the correlation between $X_1$ and $Y_1$ is .3, and the residual correlation between $X_2$ and $Y_2$ is .5.
...
$$
so that there are now two autoregressive effects and two cross-lagged effects each for both $X$ and $Y$. Note that there are no lag-2 effects, meaning that neither $X_3$ nor $Y_3$ are predicted by $X1$ and $Y1$. This assumption cannot be relaxed in `semPower.powerCLPM`. Consider power analysis for [generic path models](#powerPath) when you need lag-2 effects. 
...
To assume that the residual correlations between $X_2$ and $Y_2$ as well as $X_3$ and $Y_3$ shall be equal, add `'corXY'` to `waveEqual`. Although the constraints defined in `waveEqual` apply to both the H0 and the H1 model, this will nevertheless affect statistical power.  

Second, when $X$ and $Y$ are latent factors, the analyses models impose metric invariance constraints (equal loadings) over waves for the measurement model by default (`metricInvariance = TRUE`). This can be set to `FALSE` to disable invariance constraints over waves, however, it is generally recommended to interpret a CLPM structure only when invariance is met. In addition, the presence of invariance constrains also affects power for hypothesis concerning the CLPM parameters. However, when `metricInvariance = TRUE`, ensure to define the measurement in a way that the invariance constraints are actually met by specifying identical loadings over waves for both $X$ and $Y$. 
...
```
The results of the power analysis are printed by calling the `summary` method on `powerCLPM$power`, which in this example provides the same information as a model-free [a-priori power analysis](#ap) counterpart.
...
```
Now, `summary(powerCLPM$power)` provides the same information as a model-free [post-hoc power analysis](#ph) counterpart. A  [compromise power analysis](#cp) (`type = 'compromise'`) is performed analogously.
...
```
Note that `lavaan` throws warnings regarding the use of parameter labels, which can be safely ignored.
...
`semPower.powerRICLPM` is used to perform power analyses for hypothesis arising in random intercept cross-lagged panel models (RI-CLPM). In a standard RI-CLPM implemented here, two variables $X$ and $Y$ are repeatedly assessed at three or more different time points (waves) yielding autoregressive effects (`X1 -> X2`, `X2 -> X3`, `Y1 -> Y2`, and `Y2 -> Y3`), synchronous effects (`X1 <-> Y1`, `X2 <-> Y2` , `X3 <-> Y3`), and cross-lagged effects (`X1 -> Y2`, `Y1 -> X2`, `X2 -> Y3`, and `Y2 -> X3`). Given that the CI-CLPM is an extension of the traditional CLPM, consult the [chapter on the traditional CLPM](#powerCLPM) for a detailed description of the relevant parameters and arguments. `semPower.powerRICLPM`provides interfaces to perform power analyses concerning the following hypotheses:
...
* `nullWhich`: Defines which parameter(s) is targeted by the hypothesis defined in `nullEffect` when the parameters are not part of the `waveEqual` argument.
* `nullWhichGroups`: for hypothesis involving cross-groups comparisons, vector indicating the groups for which equality constrains should be applied.
* `metricInvariance`: whether metric invariance over waves is assumed (`TRUE`, the default) or not (`FALSE`). This generally affects power and may also affect the df, depending on the comparison model. 
* additional arguments [specifying the type of power analysis](#commonArgs). 
* additional arguments [defining the factor model](#factorDefinition), where the order of factors is ($X_1$, $Y_1$, $X_2$, $Y_2$, ..., $X_{nWaves}$, $Y_{nWaves}$). 
...
For instance, the following sets up a RI-CLPM with three waves (`nWaves = 3`) and requests the required sample (`type = 'a-priori'`) to detect that a cross-lagged effect of $X$ from wave 1 to wave 2 (`X1 -> Y1`; `nullWhich = 1`) of at least .20 (`crossedEffects = c(.20, .10)`) differs from zero (`nullEffect = 'crossedX = 0'`) on alpha = .05 (`alpha = .05`) with a power of 80% (`power = .80`). The cross-lagged effect of $Y$ is .10. The autoregressive effects of $X$ and $Y$ are .5 and .4 (`autoregEffects = c(.5, .4)`), respectively. Both the autoregressive and the cross-lagged effects of both $X$ and $Y$ are equal across waves (but freely estimated in each wave, as `waveEqual` is not set). The synchronous correlations (`rXY = NULL`) as well as the correlation between the random intercept factors (`rBXBY = NULL`) are zero. See the section on [defining the CLPM structure](#clpmDefinition) for more details on the arguments.
...
If setting `nullWhich = 2`, the cross-lagged effect of $X$ from wave 2 to wave 3 would be target of the null hypothesis. If setting `waveEqual = c('crossedX')`, both the H0 and the H1 analyses models would constrain the cross-lagged effects of $X$ to be equal across waves, so `nullWhich` can be omitted in this case, as there is only a single cross-lagged effect of $X$ left. `rXY` can be used to define synchronous correlations between $X$ and $Y$. For instance, `rXY = c(.5, .4, .3)` would define the (residual) correlation betwen $X$ and $Y$ at wave 1, 2, and 3 to equal .5, .4, and .3, respectively. `rBXBY` can be used to set the correlation between the random intercept factors. For instance, `rBXBY = .3` sets the correlation to .3. 
...
For instance, the following defines a RI-CLPM similar as in the previous section, with the exceptions that there is now a non-zero correlation between the intercept factors (`rBXBY= .30`), and the required sample size is requested to detect that this correlation differs from zero (`nullEffect = 'corBXBY = 0'`).  
...
```
Again, see the section on [defining the CLPM structure](#clpmDefinition) for more details on the arguments.

##### Detect whether the crossed effects of $X$ and $Y$ differ {-}
To perform a power analysis to detect whether the cross-lagged effect of $X$ (`X -> Y`) differs from the cross-lagged effect of $Y$ (`Y -> X`), use `nullEffect = 'crossedX = crossedY'`.  

For instance, the following sets up a RI-CLPM similar to the one defined in the previous section, with the exceptions that `crossedEffects` is now a list giving the cross-lagged effects of $X$ and $Y$ from wave to wave. The cross-lagged effect of $X$ is .20 from wave 1 to wave 2, but .10 from wave 2 to wave 3 (`c(.20, .10)`). The cross-lagged effect of $Y$ is .20 from wave 1 to wave 2, but .05 from wave 2 to wave 3 (`c(.20, .05)`). Finally, the the required sample (`type = 'a-priori'`) is requested to detect that the cross-lagged effects of $X$ and $Y$ from wave 2 to wave 3 (`nullWhich = 2`) differ (`nullEffect = 'crossedX = crossedY'`) on alpha = .05 (`alpha = .05`) with a power of 80% (`power = .80`).
...
To perform a power analysis to detect whether a cross-lagged effect of either $X$ or $Y$ differs across waves, use `nullEffect = 'crossedX'` or `nullEffect = 'crossedY'`.  

For instance, the following defines the same RI-CLPM as in the previous section, and determines the required sample size to detect that the cross-lagged effect of $X$ from wave 1 to wave 2 (of .20) differs from the cross-lagged effect of $X$ from wave 2 to wave 3 (of .10; `crossedEffects`).
...
##### Detect whether the crossed effects of either $X$ and $Y$ differ across groups  {-}
To perform a power analysis to detect whether a cross-lagged effect of either $X$ or $Y$ differs across groups, use `nullEffect = 'crossedXA = crossedXB'` or `nullEffect = 'crossedYA = crossedYB'`. 
In multigroup analysis, `autoregEffects` and/or `crossedEffects` must be provided in a list structure, where each component specifies the values for a particular group. For instance, the following sets up a CLPM with three waves (`nWaves = 3`) and requests the required sample (`type = 'a-priori'`) to detect that the cross-lagged effect of $X$ from wave 2 to wave 3 (`nullWhich = 2`) differs across groups (`nullEffect = 'crossedXA = crossedXB'`) on alpha = .05 (`alpha = .05`) with a power of 80% (`power = .80`). In the first group, the cross-lagged effects of $X$ are .25 and .10, in the second group the cross-lagged effect of $X$ are .05 and .15 from wave 1 to 2 and from wave 2 to 3, respectively (`crossedEffects`). The autoregressive effects of both $X$ and $Y$ are constant across waves and groups (but freely estimated for each wave and group). Since `semPower.powerRICLPM` by default applies metric invariance constraints across both waves and groups (`metricInvariance = TRUE`), only a single measurement model is defined that is identical in both groups. Finally, in multiple group models the `N` argument also needs to be provided as a list, which in case of an a-priori power analysis gives the group weights. `N = list(1, 1)` requests equally sized groups. If using `N = list(2, 1)` instead, the first group would be twice as large as the second group. If a post-hoc or compromise power analysis is requested, `N` is a list providing the number of observations for each group. 
...
powerRICLPM <- semPower.powerRICLPM(
                                    ...
                                    nullWhich = 2,
                                    nWaves = 3,
                                    autoregEffects = c(.5, .4),   # constant across waves and gruops
                                    crossedEffects = list(
                                    # group 1
                                    list(
                                      c(.25, .10),    # X1 -> Y2, X2 -> Y3 
