...
Whereas all convenience functions expect certain arguments (or values provided as arguments) that are unique to a specific function, a number of arguments are identical for all functions. In particular, any convenience function expects arguments [specifying the factor model](#factorDefinition) and arguments detailing the specific type of power analysis ([a-priori](#ap), [post-hoc](#ph), or [compromise](#cp)) requested. Additional common arguments are:

* `type`: The type of power analysis requested; one of `'a-priori'`, `'post-hoc'` or `'compromise'` (or shortcuts, i. e., `'ph'`, `'ap'`, `'cp'`). 
* `comparison`: The relevant comparison model; one of `'saturated'`, or `'restricted'` (the default) 
...
* `simulatedPower`: Whether to perform a simulated (`TRUE`) rather than an analytical (`FALSE`; the default) power analysis.
...
powerCFA <- semPower.powerCFA(
  ...
  # request analytical power
  simulatedPower = FALSE)
```

Whereas `nullEffect` and `nullWhich` need to be defined in any convenience function, the specifics depend on the model and hypothesis at hand, so these arguments are discussed in the relevant chapters. The [definition of the factor model]() was described previously. In the following,the `type`, `comparison`, and `simulatedPower` arguments are described.
...
By default, all power analyses are performed analytically (`simulatedPower = FALSE`). If a simulated power analysis is desired, set `simulatedPower = TRUE` and optionally define the number of replications (`nReplications`) as well as additional arguments. For instance, the following request a simulated a-priori power-analysis based on 500 replications:
...
`semPower` supports a simulation approach for all model-based power analyses. For more details on customizing the simulations, see the chapter on [simulated power](simulatedPower).
...
In the simplest case, the model contains exactly two factors, so only the to-be detected factor correlation needs to be specified (along the factor model itself). For instance, the following requests the required sample (`type = 'a-priori'`) to detect that a factor correlation of at least .25 (`Phi = .25`) differs from zero (`nullEffect = 'cor = 0'`) on alpha = .05 (`alpha = .05`) with a power of 80% (`power = .80`), where the two factors are measured by 4 and 3 indicators (`nIndicator = c(4, 3)`), respectively, and the loadings on the first and second factor are .5 and .6 (`loadM = c(.5, .6)`), respectively. See the chapter on [specifying a factor model](#factorDefinition) for alternative ways to specify the factors.
...
If using `N = list(2, 1)` instead, the first group would be twice as large as the second group. If a post-hoc or compromise power analysis is requested, `N` is a list providing the number of observations for each group. Instead of providing single numbers, `Phi` can also be a list of factor correlation matrices. 
