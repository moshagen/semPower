...
##	                                             
##	F0                       0.027416   0.027803 
##	RMSEA                    0.165577   0.166742 
##	Mc                       0.986386   0.986195 
##	GFI                      0.993945   0.993859 
##	AGFI                     0.727503   0.723677 
##	                                             
##	df                       1          1        
##	Num Observations         300        300      
##	NCP                      8.197285   8.313111 
##	                                             
##	Critical Chi-Square      3.841459   3.841459 
##	Alpha                    0.050000   0.050000 
##	Beta                     0.183229   0.182000 
##	Power (1 - Beta)         0.816771   0.818000 
##	Implied Alpha/Beta Ratio 0.272883   0.274725 
##	
##	
##	Simulation results:
##	                                                    
...
##	H0 Model                                    -0.49   
##	H1 Model                                    -1.09   
##	H0-H1 Difference                            1.22    
##	                                                    
##	Chi-Square KS-Distance                              
##	H0 Model                                    0.006130
##	H1 Model                                    0.015581
##	H0-H1 Difference                            0.007333
##	                                                    
...
The results of the power analysis provide the same information as a model-free [post-hoc power analysis](#ph), but now also include a column giving the the simulation results. Of note, the simulated power estimate (of 81.8%) closely matches the analytical result (81.7%). 

In addition, `semPower` also outputs a section entitled "Simulation results", where further results of the simulation are reported that help to evaluate whether the empirical chi-square distributions match the asympotically expected distributions and whether the realized sample size is sufficient to support parameter estimation of the model. [Recall](#ph) that power is only one of several aspects to consider in sample size planning. In particular, the achieved convergence rate of the H0 model is always provided (and should be close to 100%, as in the present example). When there is also an explicit comparison (H1) model (which is the case in the example; `comparison = 'restricted'`), the average parameter biases are also given. These information are only meaningful when the model under scrutiny fits the data, so these must refer to the H1 model. The average parameter bias shows whether the model parameters are reliably recovered and should thus be close to zero, so the parameter estimates are virtually unbiased in the present example. 

Furthermore, the empirically observed chi-square distributions are compared against the expected chi-square distributions based on asymptotic theory (i.e., the distributions analytical power analysis are based on), thereby providing a rough indication of the extent to which simulated power diverges from analytical power. The observed bias in the chi-square test-statistic compares the empirically observed mean against its expected value and thus should approach zero. The average KS distance is measure of the discrepancy between two cumulative distribution functions akin to the Kolmogorov–Smirnov test statistic. Unlike the KS-test, the average KS distance uses the mean absolute distance instead of the maximum absolute distance, so it ranges between 0 (for no difference) to 1. Both information are provided separately for the H0 model and - if available - the H1 model as well as the difference between the H0 and the H1 model. To the extent that the empirical distributions only show little departures from the theoretical distributions, analytical and simulated power will be similar. 
