...
To give a number of examples, the following provides the required sample sizes to yield a power of 80% on $\alpha = .05$, assuming that *X* and *Y* are measured by three indicators each (in each of <span style="color:red">??? how many ??? waves</span>), all loadings on *X* are .5 and those on *Y* are .7, and all autoregressive and cross-lagged effects are constant across waves and equal for *X* and *Y*:
