...
??? please check whether changes in equation are correct
...
##### Provide a single loading (??? magnitude?) to apply to indicator (??? all indicators?) of a specific factor (`nIndicator` and `loadM`)  {-}
...
??? General comment: You often write e.g., that "an indicator loads by .6". Is "by" really the correct preposition here? It sounds a bit strange to me, but I am also not sure which preposition would be better. Maybe "the indicator loads at .6?" 


## CFA models {#powerCFA}

`semPower.powerCFA` is used to perform power analyses to reject hypotheses arising in a standard CFA model involving several factors or factor(s) (??? what do you mean by "or factor(s)") and one or more additional observed covariates. `semPower.powerCFA` provides interfaces to perform power analyses concerning the following hypotheses:
...
??? As a general comment, I am not sure what you refer to when you write "bifactor". Is it only the general factor or the complete bifactor structure? Edit: It gets clear from the first subsection onwards, but it should also be clearly states in the beginning.
...
??? Maybe you could add an example on how to perform a cross-group comparison of a model with two bifactors? Some readers might wonder how the definition of the loadings would look like. Would you first need a list for the two bifactors and then an overarching list for the two groups?
...
`semPower.powerRegression` only addresses hypotheses concerning slope(s) in a latent regression (??? but you could also use it for manifest regression when defining only observed variables, right?). `semPower` provides other convenience functions for hypothesis arising in [mediation models](#powerMediation), [generic path models](#powerPath), and [cross-lagged panel models](#powerCLPM). For hypotheses regarding global model fit, a [model-free power analysis](#modelFreePower) should be performed. 
