...
The results of the power analysis provide the same information as a model-free [post-hoc power analysis](#ph), but now also include a column giving the the simulation results. Of note, the simulated power estimate (of 81.0%) closely matches the analytical result (81.7%). 

In addition, `semPower` also outputs a section entitled `Simulation results`, where further results of the simulation are reported that help to evaluate whether the empirical chi-square distributions match the asymptotically expected distributions and whether the realized sample size is sufficient to support parameter estimation of the model. [Recall](#ph) that power is only one of several aspects to consider in sample size planning. In particular, the achieved convergence rate of the H0 model is always provided (and should be close to 100%, as in the present example). When there is also an explicit comparison (H1) model (which is the case in the example; `comparison = 'restricted'`), the average parameter biases are also given. These information are only meaningful when the model under scrutiny fits the data, so these must refer to the H1 model. The average parameter bias shows whether the model parameters are reliably recovered and should thus be close to zero, so the parameter estimates are virtually unbiased in the present example. 

Furthermore, the empirically observed chi-square distributions are compared against the expected chi-square distributions based on asymptotic theory (i.e., the distributions analytical power analysis are based on) using three measures:

* `Chi-Square Bias (%)`: The observed percentage bias ($(\chi^2 - df) / df$) concerning the mean of the empirical chi-square test. Note that whereas the empirical mean usually only exhibits a rather minor departure from the expected value, the tails might differ considerably.
...
##	H1 Model                                    22.20  
```
Here, the KS-Distances indicate strong departures of the empirical distributions from the respective asymptotic distribution, which is also mirrored by a rejection rate of the (correctly specified) H1 model that clearly exceeds the alpha error of 5%. However, the difference distribution closely follows the theoretically expected distribution, so in this example there is also virtually no difference between the analytical and the simulated power estimate (omitted above). Stated differently, although the observed chi-square test-statistics are inflated under both the H0 and the H1 model, their differences sill matches the theoretical difference distribution. 
