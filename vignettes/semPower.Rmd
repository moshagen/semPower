...
In the above, we first define the correlation matrix between three factors. Then we use the `semPower.powerCFA` command to define the factors in terms of the number of indicators and the mean (and SD) loadings. Here, we are not interested in the computed power, but rather just want the generated covariance matrix of the observed variables, which we store the the variable `Sigma`.
...
# do power analysis
semPower.aPriori(alpha = .05, power = .80, df = 1, 
                 Sigma = Sigma, SigmaHat = SigmaHat)
```
