...
In SEM, the typical types of hypotheses that occur either implement equality constraints on two or more parameters (such as cross-group constraints) or assign a particular parameter a specific value (such as zero).^[A third type implements order constraints on parameters, but this is a  intricate issue.] For instance, consider a cross-lagged panel model (CLPM) with two constructs *X* and *Y* measured by three indicators each at three different waves (so the full model comprises six factors). Relevant (non-exhaustive) hypotheses could be 
...
This chapter provides a brief statistical background on hypothesis testing as well as model estimation, and effect sizes in SEM.
...
Fit-indices that are not based on non-centrality have no straightforward relation to F0 and are thus not well suited for power-analyses. However, when the input parameters include covariance matrices, `semPower` also reports the following measures.
...
When you do not expect (or are not interested in or do not have sufficiently specific hypotheses on) measurement non-invariance for certain parameters, but rather assume that non-invariance spreads across multiple parameters (say, across all loadings), consider performing model free power analysis concerning the [overall difference](#diffPower) between two models.
...
Instead of defining the true state in the population via a `lavaan` model string, it is also possible to provide the population covariance matrix directly. A useful utility function supporting this process is [semPower.generateSigma](#generateSigma). For instance, using 
```{r eval=FALSE}
...
summary(ap$power)
```
gives exactly the same results as above. See [semPower.generateSigma](#generateSigma) for more information.
...
This approach has the benefit that the result variable `pcfta` also contains generated model strings, which could be modified to reflect the actual hypothesis of interest. Instead of writing the complete H0 and H1 model strings, the above may also be achieved through
...
The output (which is omitted here) indicates that fitting the hypothesized model to the population data is  associated with a discrepancy of F0 = 0.133 (or RMSEA = .121 or SRMR = .140 or ...) and that the power to reject the H0 model is very high, 1 - beta = .993. 

It is instructive to compare the expected chi-square (calculated via the obtained F0) with the chi-square model test statistics as reported by `lavaan` when fitting the H0 model using the same number of observations as requested in the power analysis:
...
to be written...`semPower.genSigma` (#generateCov)
...
where $\mathbf{B}$ is the $m \cdot m$ matrix containing the regression slopes and $\Psi$ is the (residual) variance-covariance matrix of the \eqn{m} factors. The means are
