...
Knowledge of the degrees of freedom (df) is required to perform a power analyes. When power refers to the comparison of two explicitly specified, nested models, the resulting df are just the difference between the df of the two models (or equivalently, the number of free parameters removed by the more restrictive model). When power is requested for the comparison of a hypothesized model to the saturated model, the model df are given by 

<<<<<<< HEAD
$$df = p\cdot(p+1)/2 - q$$
=======
$$df = p\cdot(p-1)/2 - q$$
>>>>>>> f6108d257b98a38a8074fa41722cd454884dfbc0
...
Alternatively, we can use s suitable sem-package, such as `lavaan`, to determine the model df, even if we don't have any data at hand. To do so, we need to specify the hypothesized model using the lavaan standard syntax, create a dummy covariance-matrix, and then fit the model to the dummy covariance matrix. This will give warnings and non-sensical results, but we will also obtain the correct df.

```{r eval=FALSE}
library(lavaan)
# define model using indicator names from x1 to xp
model <- '
f1 =~ x1 + x2 + x3 + x4
f2 =~ x5 + x6 + x7 + x8
'
...
dummyS <- diag(p)
rownames(dummyS) <- paste0('x',1:p)
# fit model to dummy covariance matrix. This will give warnings we just can ignore
dummyFit <- sem(model, sample.cov = dummyS, sample.nobs = 10, warn = F)
# obtain df
