...
gives 19 as result, which matches what we calculated by hand.
...
Obviously, the above is a gross simplification, so `semPower.powerCFA` also understands more complex models. For example: 
```{r eval=FALSE}
...
summary(cfapower$power)
```
which shows that 645 observations are required to detect a correlation between the first two factors of at least .2 with a power of 95% on alpha = 5% (note that having additional factors that are correlated with the factors of interest helps). 
...
# Power for factor correlations ina standard CFA model

Given that obtaining the relevant covariance matrix to define an effect of interest can be rather cumbersome, `semPower` also provides the `semPower.powerCFA` that simplifies this process for the common case that the correlation between two factors in a standard CFA model is of interest.  Note that this function requires the `lavaan` package. 

Generally, `semPower.powerCFA` requires that a certain CFA model is defined in terms of the number of factors, loadings, and (population) correlations between the factors. One of these correlations defines the effect of interest. `semPower.powerCFA` then obtains the true population covariance matrix, fits a model contraining the chosen correlation to be zero, obtains the associated model-implied covariance matrix, and finally performs power analysis on these matrices. 
...
The result `cfapower` is a list comprising the results of the power analyses, the population and model-implied covariance matrices, and several lavaan model strings. To view the power output, call:
