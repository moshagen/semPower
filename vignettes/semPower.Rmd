...
By default, power analysis are performed analytically relying on asymptotic theory. Alternatively, all [model-based](#modelBasedPower) a priori and post-hoc power analyses^[Simulated power is not supported for compromise power analysis, because this required an unfeasible large number of replications to yield reliable results.] can also be performed using a simulation approach.

To request a simulated power analysis, add `simulatedPower = TRUE` as additional argument to **any** of the convenience functions performing a [model-based power analysis](#modelBasedPower) or to the `semPower.apriori` or `semPower.posthoc` functions when a covariance matrix and a model string is provided. The details of the simulation can be specified in `simOptions`, which is a list that may have the following components:

* `nReplications`: The targeted number of valid simulation runs (defaults to 250). It is recommended to use a larger number of replications as this increases accuracy (at the expense of processing time). Note that an a priori power analysis will only be accurate when the number of replications is large (say, 500 or more). 
* `minConvergenceRate`: The minimum convergence rate required (defaults to .50). The maximum actual simulation runs are increased by a factor of 1/`minConvergenceRate`, so that this argument is only used to ensure that the requested number of (successful) replications as defined in `nReplications` is actually reached, while still having a stopping rule for the simulation. `semPower` still provides the results when the actual convergence rate is smaller than `minConvergenceRate`, but will issue a warning.
* additional arguments related to ['non-normal data'](#simNonNormal) and ['missingness'](#simMissing)

For instance, the following request a simulated (`simulatedPower = TRUE`) post-hoc power analysis with default settings (i.e, 250 replications, minimum convergence rate of 50%, complete and normally distributed data, regular maximum-likelihood LRT) to detect that a factor correlation of at least .25 differs from zero on alpha = .05 with a sample of 300 observations. See the chapter on [CFA models](#powerCFA) for a description of the remaining arguments. It is strongly recommended to seed the random generator to obtain reproducable results (`set.seed(300121)`).
...
## 
##  Simulated power based on 250 successful replications.
##                                   
##  F0                       0.028249
##  RMSEA                    0.168076
##  Mc                       0.985975
...
##  Alpha                    0.050000
##  Beta                     0.180000
##  Power (1 - Beta)         0.820000
##  Implied Alpha/Beta Ratio 0.277778
## 
## 
##  Simulation results:
##                                                    
...
##                                                    
##  Chi-Square Bias (%) of the H1 model         -6.78 
##                                                    
##  Average Parameter Bias (%) in the H1 Model:       
##  Loadings                                    0.44  
##  Variances/Covariances                       0.19
```

The results of the power analysis provide the same information as a model-free [post-hoc power analysis](#ph), but are now based on the simulation results, rather than asymptotic theory. Of note, the simulated power estimate (of 82%) closely matches the power estimate of a corresponding analytical power analysis (of 81.6%; not reported here). 

In addition, `semPower` also outputs a section entitled "Simulation results", where further results of the simulations are reported that help to evaluate whether the realized sample size is sufficient to support parameter estimation of the model. [Recall](#ph) that power is only one of several aspect in sample size planning. In particular, the achieved convergence rate of the H0 model is always provided (and should be close to 100%). When there is also an explicit comparison (H1) model (which is the case in the example; `comparison = 'restricted'`), the observed bias in the chi-square test statistic and the average parameter biases are also given. These information are only meaningful when the model under scrutiny fits the data, so these must refer to the H1 model. The observed bias in the chi-square test-statistic gives a rough indication of the extent to which simulated power diverges from the analytical power (but this also depend on the bias of the chi-square test of the H0 model). The average parameter bias shows whether the model parameters are reliably recovered and thus should be close to zero.

To change the number of replicates and/or the minimum convergence rate, further supply the `simOptions` argument as a list with corresponding components. For instance, the following uses 500 replications (`nReplications = 500`) and a minimum convergence rate of 90% (`minConvergenceRate = .90`).
...
Simulated power analyses for all other convenience functions providing a [model-based power analysis](#modelBasedPower) are performed analogously.
...
powerCFA <- semPower.powerCFA(
                              ...
                              simOptions = list(
                                ...
                                kurtosis = c(20, 15, 10, 15, 6, 5, 4, 5, 5)
                              ))
```
...
One may also just define a single distribution to apply to all variables. For instance, `distributions = list('rchisq', list(df = 2))` defines the population marginal distributions for all variables to be chi-square with 2 df.    
...
ML estimation with test-statistic equivalent to the Yuan-Bentler statistic (`lavOptions = list(estimator = 'mlr')`).
