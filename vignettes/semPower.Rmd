...
To perform a power analysis to detect whether a mediation effect (= an indirect effect) differs from zero, use `nullEffect = 'ind = 0'`, which is also the default and thus could be omitted. Note again that this is yields approximate results for models involving latent variables and provide accurate results only for models without latent variables. 
...
* `standardized`: Whether all parameters are standardized (`TRUE`, the default) or unstandardized * `standardized`: Whether all parameters are standardized (`TRUE`, the default) or unstandardized (`FALSE`).
...
* `rBXBY` defines the correlation between random intercept factors (or `NULL` for no correlation).
* `nullEffect` also accepts `'corBXBY = 0'` to constrain the correlation between the random intercept factors to zero.
* Standardized (`standardized`) parameters are not available.
...
For instance, the following sets up a RI-CLPM with three waves (`nWaves = 3`) and requests the required sample (`type = 'a-priori'`) to detect that the correlation between the random intercept factor (of .30; `rBXBY= .30`) differs from zero (`nullEffect = 'corBXBY = 0'`) on alpha = .05 (`alpha = .05`) with a power of 80% (`power = .80`).  The autoregressive effects of $X$ and $Y$ are .5 and .4 (`autoregEffects = c(.5, .4)`), respectively. The cross-lagged effects of $X$ is .5, the cross-lagged effect of $Y$ .10 (`crossedEffects = c(.20, .10)`). Both the autoregressive and the cross-lagged effects of both $X$ and $Y$ are equal across waves (but freely estimated in each wave, as `waveEqual` is not set). All synchronous correlations (`rXY = NULL`) are zero. For more details on the arguments, see the chapter on traditional [CLPMs](#powerCLPM).
...
For instance, the following sets up a CLPM with three waves (`nWaves = 3`) and requests the required sample (`type = 'a-priori'`) to detect that the correlation between the random intercept factors in group 1 differs from the correlation in group 2 (`nullEffect = 'corBXBYA = corBXBYB'`) on alpha = .05 (`alpha = .05`) with a power of 80% (`power = .80`). In the first group, the correlation between the random intercept factors is .3, in the second group .6 (`rBXBY = list(.3, .6)`). There are further group differences in the cross-lagged effects (`crossedEffects`), whereas the autoregressive effects of both $X$ and $Y$ are constant across waves and groups (but freely estimated for each wave and group). For the remaining arguments, see the chapter on traditional [CLPMs](#powerCLPM).
...
`covPop` is now the covariance matrix in the population ($\Sigma$). To obtain the model implied covariance matrix ($\hat{\Sigma}$), one needs to fit our hypothesized, wrong, H0 model (`modelH0`) to the population data (`covPop`). The model-implied covariance matrix then can be obtained by calling `covH0 <- fitted(fitH0)$cov`.
