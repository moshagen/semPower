...
#' @param comparison comparison model, one of `'saturated'` or `'restricted'` (the default). This determines the df for power analyses. `'saturated'` provides power to reject the model when compared to the saturated model, so the df equal the one of the hypothesized model. `'restricted'` provides power to reject the hypothesized model when compared to an otherwise identical model that just omits the restrictions defined in `nullEffect`, so the df equal the number of restrictions.
#' @param nWaves number of waves, must be >= 2.
#' @param autoregEffects vector of the autoregressive effects, e.g. `c(.7, .6)` for  autoregressive effects of .7 for `X1 -> X2` and .6 for `X2 -> X3`. Must be a list of lists for multiple groups models.
...
#' 
#' This function only consider autoregressive models. For hypotheses regarding a CLPM structure, see [semPower.powerCLPM()].
#' 
#' Beyond the arguments explicitly contained in the function call, additional arguments 
#' are required specifying the factor model and the requested type of power analysis.  
#' 
...
semPower.powerAutoreg <- function(type, comparison = 'restricted',
  ...
  if(nullEffect == 'autoreg' && c('autoreg') %in% waveEqual) stop('You cannot set nullEffect = autoreg and waveEqual = autoreg')
  if(nullEffect == 'lag2' && c('lag2') %in% waveEqual) stop('You cannot set nullEffect = lag2 and waveEqual = lag2')
  if(nullEffect == 'lag3' && c('lag3') %in% waveEqual) stop('You cannot set nullEffect = lag3 and waveEqual = lag3')
  if(nullEffect == 'mean' && c('mean') %in% waveEqual) stop('You cannot set nullEffect = mean and waveEqual = mean')
  if(nullEffect == 'var' && c('var') %in% waveEqual) stop('You cannot set nullEffect = var and waveEqual = var')
